...
import Navbar from './event/component/Navbar'
import {encodedLetterToNumber } from "../pages/event/component/Convert"
import ShowMoreLess from './event/component/ShowMoreLess'
...
export default function Home({resServerSide}) {
  ...
  const slug = router.query.slug != undefined ? router.query.slug : ''
                      ...
                      <div className='lg:w-4/5 mx-auto flex flex-wrap'>
                          ...
                          <div className='lg:w-1/3 w-full lg:h-auto h-auto object-cover object-center rounded-md'>
                            <Carousel showThumbs={false} autoPlay={true} showArrows={false} infiniteLoop={true} showIndicators={photoes.length !== 1} showStatus={photoes.length !== 1} className='rounded-lg bg-black'>
                              {photoes.map((e, index) => (
                                <>
                                {fileExt(e) == 'video'?
                                <video className="bg-video__content h-80" autoPlay muted loop id="myVideo">
                                  <source src={e} type="video/mp4"/>
                                  {/* <source src="/image/file_example_MOV_480_700kB.mov" type="video/mp4"/> */}
                                </video>
                                :
                        ...
                        <div className='lg:w-1/2 w-full lg:pl-10 mt-3 lg:mt-0 '>
                          <h2 className='mb-2'  onCut={handleChange} onCopy={handleChange} onPaste={handleChange}>
                            <span className='text-gray-800 text-xs font-medium mr-2 px-2.5 py-1 rounded-full border border-gray-500 dark:text-white '> {eventTicketDetail.event_type} </span>
                          </h2>
                          <h1 className='text-gray-900 text-3xl title-font font-medium mb-2 dark:text-white'>{eventTicketDetail.event_name}</h1>
                          ...
                          <span className='flex items-center  mb-2 mt-2'>
                            ...
                            <span className='bg-gray-100 px-2.5 py-1.5 rounded dark:bg-black ml-3 text-xs'>
                          ...
                          <div className='flex mb-2'>
                            <span className='flex items-center'>
                              ...
                              <span className='bg-gray-100 px-2.5 py-1.5 rounded dark:bg-black mr-3 text-xs'>
                          ...
                          {usedTicketCount > 0 && (
                            <div className='flex -space-x-4 items-center mt-6 block max-w-sm p-2 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-white' onCut={handleChange} onCopy={handleChange} onPaste={handleChange}>
                                  ...
                                  {e.photo ? (
                                    ...
                                    <>
                                      <div className='relative w-10 h-10 overflow-hidden rounded-full'>
                                        <a className='flex items-center justify-center w-10 h-10 text-xs font-medium text-white  border-2 border-white rounded-full hover:bg-gray-600 text-center uppercase dark:text-black' title={e.first_name + ' ' + e.last_name} style={{ background: AvatarColors[index] }}>
                              ...
                              <span className='h5 pl-5 mb-0 f-bold dark:text-black'>
                                {' '}
                          ...
                          {eventTicketDetail.event_about && (
                            <div className='mt-5'>
                              <label className='ml-1 text-gray-800 text-lg font-semibold dark:text-white'>About the Event</label>                             
                              <ShowMoreLess className="dark:text-white !important" about={eventTicketDetail.event_about}/>                          
                              
                            </div>
                          )}

                          <div className='mb-5 mt-2 ml-1'>
                            <label className='text-gray-800 text-lg font-semibold dark:text-white'>Organizer</label>
                            <span className='flex items-center mr-3'>
                              <span className='text-gray-600 dark:text-white' title='Orgnizer'>
                                {eventTicketDetail.organizer}
                              </span>
                            </span>

                            <div className='mt-2'>
                              <label className='text-gray-800 text-lg font-semibold dark:text-white'>About the Organizer</label>
                              <span className='flex items-center mr-3'>
                                <span className='text-gray-600 dark:text-white' title='Orgnizer E-mail'>
                        ...
                        <div className='container px-1 py-0 mx-auto lg:w-3/6'>
                          ...
                          {isStartTimer && (
                            <div className='text-center mb-3'>
                              ...
                              <p className='font-medium text-current text-xs lg:text-sm dark:text-white'>Your ticket will be held for 5mins, after which it’ll be released for sale.</p>
                            </div>
                          )}
                          ...
                          {eventTicketDetail.tickets.map((item, index) => {
                            ...
                            return (
                              <div className={`shadow p-3 mb-5 bg-white rounded flex-col ${valueAfterFilter.length > 0 ? `border border-violet-700 dark:border-2 border-violet-700` : `border dark:border`}`}  key={index}>
                                <div className='align-items-start mb-1 columns-2 relative'>
                                  <div className='relative'>
                                    <span className='text-[0.8125rem] lg:text-base font-semibold block text-[#32325d]'>{ticket_name}
                                    <p className='text-base lg:text-md  font-semibold mb-0 block text-[#32325d]'>{ticket_type === 1 ? `$${parseFloat(ticket_price).toFixed(2)}` : 'Free'}</p>
                          ...
                          <div className='row mt-5'>
                            <div className='col-sm-12 mt-3'>
                              ...
                              {eventTicketDetail?.is_base_fee === 0 && (
                                <span className='font-medium ml-4 text-base dark:text-white'>
                                  Fee: &nbsp;&nbsp;
                            ...
                            <>
                              {promoCode.length > 0 && promoCodeDiscount > 0 ? (
                                <div className={`promo-view mt-4  p-2  ${promoCode != '' ? `border border-violet-700` : `border`} shadow rounded relative`}>
                                  <span className='font-bold mr-4 dark:text-white'>
                                    {promoCodeDiscount}% applied (-$
                                    {parseFloat((totalPrice * promoCodeDiscount) / 100).toFixed(2)})
                                  </span>
                                  <button className='btn btn-remove bg-red-600 text-white absolute outline-0 right-2 top-1.5 px-5 py-0.5 rounded dark:text-white' onClick={onHandleRemovePromo}>
                                    Remove
                                  </button>
                                </div>
                              ) : (
                                ...
                                <div className={`promo-view mt-3 ${promoCode != '' ? `border border-violet-700` : `border`} shadow rounded p-1 relative`}>
                                  ...
                                  <Input className='form-control outline-0 p-2 remove focus:outline-none dark:text-white dark:placeholder-white' placeholder='Promo Code' onChange={(e) => setPromoCode(e.target.value)} value={promoCode} autoCapitalize={'none'} maxLength={7} />
                          ...
                          <div className='row mt-2'>
                            <div className='col-sm-12'>
                              <Input className='form-control outline-0 p-2 border w-full rounded focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='Email' onChange={(e) => setEmail(e.target.value)} value={email} autoCapitalize={'none'} />
                            </div>
                          </div>
                          <div className='row mt-2'>
                            <div className='col-sm-12'>
                              <Input className='form-control outline-0 p-2 border w-full rounded mt-1 focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='Confirm Email' onChange={(e) => setConfirmEmail(e.target.value)} value={confirmEmail} autoCapitalize={'none'} />
                            </div>
                          </div>
                          {/* -- More fields --- */}
                          <div className='row mt-3'>
                            <div className='col-sm-12'>
                              <Input className='form-control outline-0 p-2 border w-full rounded focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='Phone' onChange={(e) => setPhone(e.target.value)} value={phone} autoCapitalize={'none'} />
                            </div>
                          </div>
                          <div className='mt-3 columns-2'>
                            <div className='col-sm-6'>
                              <Input className='form-control outline-0 p-2 border w-full rounded focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='First name' onChange={(e) => setFirstName(e.target.value)} value={first_name} autoCapitalize={'none'} />
                            </div>
                            <div className='col-sm-6'>
                              <Input className='form-control outline-0 p-2 border w-full rounded focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='Last name' onChange={(e) => setLastName(e.target.value)} value={last_name} autoCapitalize={'none'} />
                            </div>
                          </div>
                          ...
                          {parseFloat((totalPrice * (100 - promoCodeDiscount)) / 100) > 0 && first_name !== '' && last_name !== '' && email !== '' && phone !== '' && (
                            <>                              
                              {paymentRequest && isApplePay && (
                                <>
                                  <PaymentRequestButtonElement className='mt-3' options={{ paymentRequest }} />
                                  <div class="inline-flex items-center justify-center w-full  mt-5">
                                      <hr class="w-64 h-1 my-8 bg-gray-200 border-0 rounded dark:bg-violet-700" />
                                      <div class="absolute px-4 -translate-x-1/2 bg-white left-1/2 dark:bg-black dark:text-white font-semibold">
                                      Or Pay with card
                                      </div>
                                  </div>
                                </>
                              ...
                              <div className='row mt-3'>
                                <div className='col-sm-12'>
                                  <Input className='outline-0 p-2 border w-full rounded focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='Credit card number' onChange={(e) => setCardNo(cardNumberFormat(e.target.value))} value={card_no} autoCapitalize={'none'} maxLength={22} />
                                </div>
                              </div>
                              <div className='flex'>
                                <div className='col-sm-4 mr-1 w-full'>
                                  <Input className='outline-0 p-2 border w-full rounded mt-3 focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='MM/YY' onChange={(e) => setCardExp(cardExpFormat(e.target.value))} value={card_exp} autoCapitalize={'none'} maxLength={5} />
                                </div>
                                <div className='col-sm-4 mr-1 w-full'>
                                  <Input className='outline-0 p-2 border w-full rounded mt-3 focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='CVV' onChange={(e) => setCardCvv(e.target.value)} value={card_cvv} autoCapitalize={'none'} maxLength={4} />
                                </div>
                                <div className='col-sm-4 w-full'>
                                  <Input className='outline-0 p-2 border w-full rounded mt-3 focus:outline-none focus:border-violet-700 dark:text-white dark:placeholder-white' placeholder='ZIP' onChange={(e) => setZip(e.target.value)} value={zip} autoCapitalize={'none'} maxLength={8} />
                                </div>
                              </div>
                              
                              <div className='mt-4 dark:text-white' style={{ fontSize: '0.8125rem' }}>
